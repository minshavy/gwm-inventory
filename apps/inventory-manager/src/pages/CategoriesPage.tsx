...
export default function CategoriesPage() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await saveLookup({ type: 'category', id: editId, name: form.name, prefix: form.prefix || undefined, description: form.description || undefined });
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await deleteLookup({ type: 'category', id: deleteId });
  ...
  return (
    <div className="space-y-6">
          ...
          {categories.map(c => (
            <div key={c.id} className="flex items-center justify-between px-4 py-3">
              <div className="flex items-center gap-3">
                <Badge variant="secondary">{c.prefix || c.name.slice(0, 3).toUpperCase()}</Badge>
                <div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4">
            ...
            <div><Label>Prefix</Label><Input value={form.prefix} onChange={e => setForm(f => ({ ...f, prefix: e.target.value }))} placeholder="e.g. PRF" maxLength={5} /></div>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={open => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Delete Category</AlertDialogTitle><AlertDialogDescription>This will delete this category permanently.</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter>
